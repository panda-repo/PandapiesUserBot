...
@Pandapies.on_message(filters.command("cinfo", prefixes=HANDLER) & filters.user(OWNER_ID))
async def cinfo(_, m):
    ...
    if image:
        photo = await Pandapies.download_media(image.big_file_id)
        text = f"✪ **TYPE:** Channel\n\n"
        text += f"✪ **ID:** {id}\n\n"
        text += f"✪ **NAME:** {name}\n\n"
        text += f"✪ **USERNAME:** @{username}\n\n"
...
@Pandapies.on_message(filters.command("info", prefixes=HANDLER) & filters.user(OWNER_ID))
async def info(_, m):
    m.reply_to_message
    if len(m.command) < 2:
    ...
    info = await Pandapies.get_chat(id_user)
    if info.photo:
        file_id = info.photo.big_file_id
        photo = await Pandapies.download_media(file_id)
        user_id = info.id
